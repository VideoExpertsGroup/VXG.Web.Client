...
list($userID,$cameraID) = MCore::checkAndGetInputParameters(['user_id', 'cam_id']);

$user = MUser::getUserById(0+$userID);

$camera = MCamera::getCameraByChannelIdAndUser($cameraID, $user);
...
if ($camera){
    ...
    MCamera::removeByChannelID($cameraID, $user);

$user->updateAllCamsToken();

if (!$user->allCamsToken)
    MCore::$core->response['allCamsToken'] = null;
else
    MCore::$core->response['allCamsToken'] = $user->allCamsToken;
